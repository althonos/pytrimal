...
from pytrimal import Alignment, BaseTrimmer, AutomaticTrimmer, ManualTrimmer, OverlapTrimmer
...
STATISTIC: Dict[ str, Callable[["TRIMMER_BACKEND"], BaseTrimmer] ] = {
    ...
    "Similarity": lambda backend: ManualTrimmer(similarity_threshold=0.5, backend=backend),
    "Overlap": lambda backend: OverlapTrimmer(sequence_overlap=60, residue_overlap=0.5, backend=backend)
}
...
with rich.progress.Progress(transient=True) as progress:
    ...
    for statistic, get_trimmer in progress.track(STATISTIC.items(), task_id=task0):
        ...
        for backend in progress.track(backends, task_id=task1):
            ...
            task2 = progress.add_task(total=len(subsets), description=f"  Subset (0/{len(example.sequences)})")
            for subset in progress.track(subsets, task_id=task2):
                progress.update(task_id=task2, description=f"  Subset ({subset}/{len(example.sequences)})")
                times = []
                alignment = Alignment(example.names[:subset], example.sequences[:subset])
                task3 = progress.add_task(total=args.runs, description=f"  Repeat (0/{args.runs})")
                for run in progress.track(range(args.runs), task_id=task3):
                    progress.update(task_id=task3, description=f"  Repeat ({run}/{args.runs})")
                    t1 = time.time()
