...
public:
    ...
    inline uint16_t sum() const {
        ...
        return _mm256_extract_epi8(vsum, 0) + _mm256_extract_epi8(vsum, 4) + _mm256_extract_epi8(vsum, 8)+ _mm256_extract_epi8(vsum, 12);
    }

    inline void clear() {
        vector = _mm256_setzero_si256();
    }
};
...
bool AVXCleaner::calculateSpuriousVector(float overlap, float *spuriousVector) {
    StartTiming("bool AVXCleaner::calculateSpuriousVector(float overlap, float *spuriousVector) ");
    return simd::calculateSpuriousVector<AVXVector>(*this, overlap, spuriousVector);
}
