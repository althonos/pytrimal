...
int pyreadbuf::underflow() {
    PyObject* bytes = PyObject_CallMethodObjArgs(handle, method, bufsize_py, NULL);
    if (bytes == nullptr) {
        return EOF;
    }
    ...
    if (size <= 0) {
        ...
        // error
        Py_DECREF(bytes);
        PyErr_SetString(PyExc_BufferError, "more data returned by `read` than can fit in buffer");
        return EOF;
    } else {
