...
class TestManualTrimmer(TrimmerTestCase, unittest.TestCase):
    ...
    def _test_parameters(self, gt, cons):
        ...
        trimmer = ManualTrimmer(gap_threshold=gt, conservation_percentage=cons, platform=self.platform)
    ...
    def test_window(self):
        ...
        trimmer = ManualTrimmer(gap_threshold=0.9, window=3, platform=self.platform)
        trimmed = trimmer.trim(ali)
    ...
    def test_large_window(self):
        ...
        trimmer = ManualTrimmer(gap_threshold=0.9, window=100, platform=self.platform)
    ...
    def test_repr(self):
        ...
        trimmer = ManualTrimmer(window=5, platform=None)
        self.assertEqual(repr(trimmer), "ManualTrimmer(window=5, platform=None)")
        ...
        self.assertEqual(
            repr(trimmer),
            "ManualTrimmer(gap_absolute_threshold=10, similarity_threshold=0.5, conservation_percentage=50.0, gap_window=5, similarity_window=5, platform=None)",
        )

    def test_pickle(self):
        trimmer = ManualTrimmer(gap_threshold=0.4, window=5, platform=self.platform)
...
class TestManualTrimmerSSE2(TestManualTrimmer):
    platform = "sse2"


@unittest.skipUnless(_trimal._AVX2_RUNTIME_SUPPORT, "AVX2 not available")
class TestManualTrimmerAVX2(TestManualTrimmer):
    platform = "avx2"
